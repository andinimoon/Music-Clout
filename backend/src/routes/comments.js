...
router.get(
  ...
  wrapAsync(async (req, res) => {
    ...
    const payload = await CommentsDBApi.findAll(req.query, { currentUser });
    if (filetype && filetype === 'csv') {
      const fields = ['id', 'content', 'comment_date'];
      const opts = { fields };
      try {
...
router.get(
  ...
  wrapAsync(async (req, res) => {
    const currentUser = req.currentUser;
    const payload = await CommentsDBApi.findAll(req.query, null, {
      countOnly: true,
      currentUser,
    });
